...
M.setup_heir = function()
  if require("lazy.core.config").plugins["heirline.nvim"] then
    ...
    local status_lsp_prefix = " "
    local status_lsp_show_server_names = false
    ...
    local LSPActive = {
      ...
      provider = function()
        ...
        local nr_of_names = tablelength(names)
        local prefix = status_lsp_prefix
        if status_lsp_show_server_names then
